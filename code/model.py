...
class Classifier(nn.Module):
    def __init__(self, input_size):
        ...
        # positional_encoding = get_positional_encoding(740)
    ...
    def forward(self, x):
        ...
        # positional_encoding = self.positional_encoding.squeeze(0).to(x.device)
        # x = torch.matmul(x, positional_encoding.T)
        x = self.fc1(x)
        x = torch.sigmoid(x)
        return x
