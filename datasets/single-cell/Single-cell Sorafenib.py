...
source_data = pd.read_csv('/data/sr/Sorafenib.csv')
target_data = pd.read_csv('/data/sr/Target_expr_resp_z.Sorafenib_tp4k.csv')
...
t_1 = target_data[target_data['label'] == 16].iloc[:, 3:].values
...
t_r = target_data[target_data['label'] == 16].iloc[:, 1].values
...
for i in range(16):
    # 初始化一个全零张量
    zeros_tensor = torch.zeros((l[i].shape[0], 16))
    # 在第 i 列插入全一张量
    zeros_tensor[:, i] = 1
    # 将生成的张量加入到结果列表中
    l[i] = zeros_tensor
zeros_tensor = torch.zeros((l[16].shape[0], 16))
zeros_tensor[:, 15] = 1
l[16] = zeros_tensor
# print(x['x3_label'])
# 将列表中的张量连接起来，沿着列的方向
...
pd.DataFrame(tlabel_).to_csv('SC_Sorafenib_tlable.csv')
pd.DataFrame(tpred_).to_csv('SC_Sorafenib_tprde.csv')
pd.DataFrame(total_feature).to_csv('SC_Sorafenib_features.csv')
pd.DataFrame(auc_scores).to_csv('auc_Sorafenib.csv')
pd.DataFrame(tauc_scores).to_csv('tauc_Sorafenib.csv')
