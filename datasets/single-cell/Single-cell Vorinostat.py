...
source_data = pd.read_csv('/data/sr/Vorinostat.csv')
target_data = pd.read_csv('/data/sr/Target_expr_resp_z.Vorinostat_tp4k.csv')
...
optimizer = torch.optim.Adam(model.parameters(), lr=8e-5, weight_decay=2e-5)
...
# pd.DataFrame(tlabel_).to_csv('Vorinostat_tlabel.csv')
# pd.DataFrame(tpred_).to_csv('Vorinostat_tpred.csv')
# pd.DataFrame(total_feature).to_csv('Vorinostat_features.csv')
# pd.DataFrame(total_ic).to_csv('PLX4720_ic_x16.csv')
# print(tlabel_)
# print(tpred_)
# pd.DataFrame(auc_scores).to_csv('Vorinostat_auc.csv')
# pd.DataFrame(tauc_scores).to_csv('Vorinostat_tauc.csv')
plt.figure()
lw = 2
...
plt.ylim(0, 1)
# 添加标题和轴标签
plt.title('Vorinostat without LIP AUC Scores over Epochs')
plt.xlabel('Epoch')
