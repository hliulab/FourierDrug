...
source_data = pd.read_csv('/data/sr/Cetuximab.csv')
target_data = pd.read_csv('/data/sr/Target_expr_resp_z.Cetuximab_tp4k.csv')
...
# pd.DataFrame(tlabel_).to_csv('SC-Cetuximab_tlable.csv')
# pd.DataFrame(tpred_).to_csv('SC-Cetuximab_tprde.csv')
# pd.DataFrame(total_feature).to_csv('SC-Cetuximab_features.csv')
...
plt.ylim(0, 1)
# 添加标题和轴标签
plt.title('Cetuximab with LIP AUC Scores over Epochs')
plt.xlabel('Epoch')
