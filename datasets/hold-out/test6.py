...
source_data = pd.read_csv('/data/sr/updated_output5.csv')
target_data = pd.read_csv('/data/sr/Target_expr_resp_z.Afatinib_tp4k.csv')
...
for i in range(22):
    ...
    # 将生成的张量加入到结果列表中
    l[i] = zeros_tensor
zeros_tensor = torch.zeros((l[21].shape[0], 22))
zeros_tensor[:, 21] = 1
l[21] = zeros_tensor
# print(x['x3_label'])
# 将列表中的张量连接起来，沿着列的方向
...
s1 = torch.Tensor(s1)
s2 = torch.Tensor(s2)
# ic = np.concatenate((ic_1, ic_2, ic_3, ic_4, ic_5, ic_6, ic_7, ic_8, ic_9, ic_10, ic_11, ic_12, ic_13, ic_14, ic_15, ic_16, ic_17, ic_18, ic_19, ic_20, ic_21),axis=0)
ic = torch.Tensor(ic_21)
# t1 = torch.Tensor(t1)
# t2 = torch.Tensor(t2)
r1 = torch.Tensor(r1)
r2 = torch.Tensor(r2)
...
# ic_0 = ic_0.detach().cpu().numpy()
total_labels = total_labels.cpu().numpy()
total_feature = total_feature.cpu().numpy()
# total_ic = total_ic.cpu().numpy()
tpr, fpr, _ = roc_curve(tlabel_, tpred_)
# pd.DataFrame(tlabel_).to_csv('Afatinib_tlable_x21.csv')
# pd.DataFrame(tpred_).to_csv('Afatinib_tprde_x21.csv')
# pd.DataFrame(total_ic).to_csv('Afatinib_ic50_x21.csv')
# pd.DataFrame(features).to_csv('features of Afatinib.csv')
# pd.DataFrame(ic_0).to_csv('ic50 of Afatinib.csv')
plt.figure()
lw = 2
...
plt.ylim(0, 1)
# 添加标题和轴标签
plt.title('Afatinib without LIP AUC Scores over Epochs')
plt.xlabel('Epoch')
...
#
# plt.legend()
# plt.title('features of Afatinib')
# plt.savefig('Afatinib_ex21.jpg', dpi=500)
# total_labels = total_labels.cpu().numpy()
# total_feature = total_feature.numpy()
# total_response = total_response.numpy()
# pd.DataFrame(total_labels).to_csv('Afatinib_total_labels_ex21.csv')
# pd.DataFrame(total_feature).to_csv('Afatinib_total_feature_ex21.csv')
# pd.DataFrame(total_response).to_csv('Afatinib_total_response_ex21.csv')
